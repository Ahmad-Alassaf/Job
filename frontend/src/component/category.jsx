...
const Category = () => {
  ...
  : 'http://localhost:8000';
    ...
    const newCategory=async(e)=>{
        e.preventDefault()
        try {
            const response=await axios.post(`${myServer}/api/categories`,{categoryTxt},{headers})
            if(!response)
            {
    ...
    const handleDeleteCategory=async(id)=>{
        ...
        try {
            ...
            const response=await axios.delete(`${myServer}/api/categories/${id}`,{headers})
            ...
            if(!response)
            {
                ...
                console.log(response.data)
                 ...
                 const updated = await getCategories(myServer, headers);
    ...
    useEffect(()=>{
       const fetchData = async () => {
                const categories = await getCategories(myServer, headers);
